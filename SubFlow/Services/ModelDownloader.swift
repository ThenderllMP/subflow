...
/// upstream-controlled, there is no self-hosted asset MeetingFlow has to keep in
...
///
/// Layout on disk:
///   ~/Library/Application Support/MeetingFlow/MoonshineModels/<modelId>/
///     ├── adapter.ort
///     ├── cross_kv.ort
///     ├── decoder_kv.ort
...
enum ModelDownloader {
    ...
    static var modelsDirectory: URL {
        FileManager.default
            ...
            .appendingPathComponent("MeetingFlow/MoonshineModels", isDirectory: true)
    }
    ...
    static func migrateLegacyModelsIfNeeded() {
        let appSupport = FileManager.default
            .urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let newDir = appSupport.appendingPathComponent("MeetingFlow/MoonshineModels")
        ...
        do {
            ...
            AppLogger.log("Migrated models to MeetingFlow")
        } catch {
...
extension ModelSource {
    ...
    /// Pointing MeetingFlow at that CDN directly avoids mirroring model weights on
